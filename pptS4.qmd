...
3.  Lecture d'un tableau croisé et test du Khi2

4.  Introduction aux méthodes de régression
...
# Le test du Khi2 pour les variables catégorielles à plusieurs modalités (exemple sans pondération)

Le test du Khi2 repose sur une idée simple : si les effectifs du tri croisé de deux variables sont très différents des effectifs qu'on devrait observer si les deux variables étaient indépendantes, alors on peut raisonnablement conclure qu'elles n'ont pas d'effets l'une sur l'autre. Voilà pourquoi on part d'abord du [tableau de contingence]{style="color:#FF7F50"} qui présente les [effectifs observés]{style="color:#FF7F50"} du croisement de deux variables.
